...
static esp_err_t bme680_set_mode(BME680_t *dev, uint8_t mode)
{
    ...
    read_reg_8_nolock(dev, BME680_REG_CTRL_MEAS, &reg);
    reg = bme_set_reg_bit(reg, BME680_MODE, mode);
    write_reg_8_nolock(dev, BME680_REG_CTRL_MEAS, reg);
...
static esp_err_t bme680_get_raw_data(BME680_t *dev, bme680_raw_data_t *raw_data)
{
    if (!dev->meas_started)
    {
        ESP_LOGE(TAG, "La medicion no se ha iniciado");
    ...
    if (!(dev->meas_status & BME680_NEW_DATA_BITS))
    {
        ...
        read_reg_8(dev, BME680_REG_MEAS_STATUS_0, &dev->meas_status);
        // test whether there are new data
        if (!(dev->meas_status & BME680_NEW_DATA_BITS))
        {
            if (dev->meas_status & BME680_MEASURING_BITS)
            {
                ESP_LOGW(TAG, "La medicion aun esta en marcha");
                return ESP_ERR_INVALID_STATE;
            }
            ESP_LOGW(TAG, "No hay datos nuevos");
    ...
    bme_i2c_dev_read_reg(dev, BME680_REG_RAW_DATA_0, raw, BME680_REG_RAW_DATA_LEN);
...
{
	if (addr != BME680_DEV_ADDR_0 && addr != BME680_DEV_ADDR_1)
	{
		ESP_LOGE(TAG, "Direccion I2C no valida");
		return ESP_ERR_INVALID_ARG;
	}

    return bme_i2c_init(master_bus, dev, addr);
}
...
esp_err_t bme680_init_sensor(BME680_t *dev)
{
    ...
    // reset the sensor
    write_reg_8_nolock(dev, BME680_REG_RESET, BME680_RESET_CMD);
    vTaskDelay(pdMS_TO_TICKS(BME680_RESET_PERIOD));

    uint8_t chip_id = 0;
    read_reg_8_nolock(dev, BME680_REG_ID, &chip_id);
    if (chip_id != 0x61)
    {
    ...
    bme_i2c_dev_read_reg(dev, BME680_REG_CD1_ADDR, buf + BME680_CDM_OFF1, BME680_REG_CD1_LEN);
    bme_i2c_dev_read_reg(dev, BME680_REG_CD2_ADDR, buf + BME680_CDM_OFF2, BME680_REG_CD2_LEN);
    bme_i2c_dev_read_reg(dev, BME680_REG_CD3_ADDR, buf + BME680_CDM_OFF3, BME680_REG_CD3_LEN);
    ...
    bme680_set_oversampling_rates(dev, BME680_OSR_1X, BME680_OSR_1X, BME680_OSR_1X);
...
esp_err_t bme680_force_measurement(BME680_t *dev)
{
    if (dev->meas_started)
    {
        ESP_LOGE(TAG, "La medicion ya esta en marcha");
        return ESP_ERR_INVALID_STATE;
    }

    // Set the power mode to forced mode to trigger one TPHG measurement cycle
    bme680_set_mode(dev, BME680_FORCED_MODE);
    dev->meas_started = true;
    dev->meas_status = 0;

    ESP_LOGI(TAG, "Medicion iniciada");
...
esp_err_t bme680_get_results_fixed(BME680_t *dev, bme680_values_fixed_t *results)
{
    ...
    if (dev->settings.heater_profile != BME680_HEATER_NOT_USED)
    {
        ...
        if (raw.gas_valid && raw.heater_stable)
            ...
            ESP_LOGW(TAG, "Los datos del gas no son validos.");
        else
            ESP_LOGW(TAG, "El calentador no es estable");
    }
...
esp_err_t bme680_get_results_float(BME680_t *dev, BME680_values_float_t *results)
{
    ...
    bme680_get_results_fixed(dev, &fixed);
...
esp_err_t bme680_measure_fixed(BME680_t *dev, bme680_values_fixed_t *results)
{
    ...
    if (duration == 0)
    {
        ESP_LOGE(TAG, "No se pudo obtener la duracion de la medicion");
        return ESP_FAIL;
    }
                                        ...
                                        bme680_oversampling_rate_t osp, bme680_oversampling_rate_t osh)
{
    ...
    {
        // read the current register value
        read_reg_8_nolock(dev, BME680_REG_CTRL_HUM, &reg);

        // set changed bit value
        reg = bme_set_reg_bit(reg, BME680_OSR_H, osh);

        // write back the new register value
        write_reg_8_nolock(dev, BME680_REG_CTRL_HUM, reg);
    }
...
esp_err_t bme680_set_filter_size(BME680_t *dev, bme680_filter_size_t size)
{
    ...
    // read the current register value
    read_reg_8_nolock(dev, BME680_REG_CONFIG, &reg);
    // set changed bit value
    reg = bme_set_reg_bit(reg, BME680_FILTER, size);
    // write back the new register value
    write_reg_8_nolock(dev, BME680_REG_CONFIG, reg);
...
esp_err_t bme680_set_heater_profile(BME680_t *dev, uint8_t profile, uint16_t temperature, uint16_t duration)
{
    ...
    if (temperature_changed)
        write_reg_8_nolock(dev, BME680_REG_RES_HEAT_BASE + profile, heat_res);
    if (duration_changed)
        write_reg_8_nolock(dev, BME680_REG_GAS_WAIT_BASE + profile, heat_dur);
...
esp_err_t bme680_use_heater_profile(BME680_t *dev, int8_t profile)
{
    ...
    write_reg_8_nolock(dev, BME680_REG_CTRL_GAS_1, reg);
...
esp_err_t bme680_set_ambient_temperature(BME680_t *dev, int16_t ambient)
{
    ...
    bme_i2c_dev_write_reg(dev, BME680_REG_RES_HEAT_BASE, data, 10);
